<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
<title>내장객체 = response</title>
</head>
<body>
	<h3>1.로그인 폼</h3>
	<form method="post" action="08_responseLogin.jsp">
	<label>아이디:
		<input type = "text" name="user_id"><br>
	</label>
	<label>비밀번호:
		<input type = "password" name="user_pwd"><br>
	</label>
	<input type="submit" value="로그인">
	</form>
	
	
	<%
		String errStr = "";
		String loginErr = request.getParameter("loginErr");
		if (loginErr != null){
			errStr = "로그인 실패";
		}
	%>
	<span style='color:red'> <%= errStr %></span>
	
	<br><br>
	<h3>2.HTTP 응답 헤더 설정하기</h3>
	<form action = "09_responseHeader.jsp" method="get">
		날짜 형식 : <input type="text" name="add_date" value="2024-02-06 09:00"><br>
		숫자 형식 : <input type="text" name="add_int" value="1234"><br>
		문자 형식 : <input type="text" name="add_str" value="홍길동"><br>
		<input type="submit" value="응답 헤더 설정 및 출력">
	</form>
</body>
</html>